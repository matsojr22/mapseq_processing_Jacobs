...
for age in age_groups:
    # Look for aggregate upsetplot files (matching pattern: {age}_ALL_HAN_filters_upsetplot.csv or {age}_alL_HAN_filters_upsetplot.csv)
    # Search in subdirectories like p12/05.HAN_filter_parameters_*/analysis/
    # Try both uppercase and lowercase variations
    patterns = [
        str(OUTPUT_DIR / age / "**" / f"*ALL_HAN_filters_upsetplot.csv"),
        str(OUTPUT_DIR / age / "**" / f"*alL_HAN_filters_upsetplot.csv"),
        str(OUTPUT_DIR / age / "**" / f"{age.upper()}_ALL_HAN_filters_upsetplot.csv"),
        str(OUTPUT_DIR / age / "**" / f"{age.lower()}_ALL_HAN_filters_upsetplot.csv"),
        str(OUTPUT_DIR / age / "**" / f"{age.upper()}_alL_HAN_filters_upsetplot.csv"),
        str(OUTPUT_DIR / age / "**" / f"{age.lower()}_alL_HAN_filters_upsetplot.csv"),
    ]
    files = []
    ...
    if not files:
        print(f"Warning: No aggregate upsetplot file found for {age}")
        age_motif_data[age] = {}
        continue
    ...
    print(f"  Loaded {len(motif_percentages)} motifs for {age}")
...
# Save the generated matrix for reference
output_csv = OUTPUT_SCRIPT_DIR / "motif_percent_matrix_by_age.csv"
matrix_df.to_csv(output_csv)
print(f"\nGenerated and saved motif_percent_matrix_by_age.csv to: {output_csv}")
...
# === Prepare PDF Outputs ===
pdf_path_sorted = OUTPUT_SCRIPT_DIR / "motif_barplots_sorted.pdf"
pdf_path_by_rank = OUTPUT_SCRIPT_DIR / "motif_barplots_ranked.pdf"
...
histogram_file = OUTPUT_SCRIPT_DIR / "histogram_similarity_summary.txt"
...
if js_data:
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_SCRIPT_DIR / "js_divergence_histogram_comparisons.png")
    plt.close()

# === Per-Motif Percent Change Analysis Across Ages ===
transition_file = OUTPUT_SCRIPT_DIR / "motif_transition_significance_summary.txt"
...
plt.savefig(OUTPUT_SCRIPT_DIR / "hclust_heatmap.png")
plt.close()
...
plt.tight_layout()
plt.savefig(OUTPUT_SCRIPT_DIR / "pca_kmeans_plot.png")
plt.close()
...
pca_loadings.T.to_csv(OUTPUT_SCRIPT_DIR / "pca_feature_loadings.csv")
...
pca_df.to_csv(OUTPUT_SCRIPT_DIR / "motif_pca_clusters.csv")
