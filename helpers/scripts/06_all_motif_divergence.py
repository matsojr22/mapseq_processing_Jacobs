...
# Look for input file in the 01_motif_analysis output directory
output_01_dir = script_dir.parent / "outputs" / "05_motif_analysis"
input_path = output_01_dir / "motif_transition_significance_summary.txt"
real_comparison_file = script_dir / "motif_real_p12vsP60.tsv"

# If not found in output directory, try script directory
if not input_path.exists():
    alt_path = script_dir / "motif_transition_significance_summary.txt"
    if alt_path.exists():
        input_path = alt_path
    else:
        raise FileNotFoundError(
            f"Could not find motif_transition_significance_summary.txt. "
            f"Expected location: {output_01_dir / 'motif_transition_significance_summary.txt'}. "
            f"Run 01_motif_analysis.py first to generate this file."
        )

input_path = str(input_path)
real_comparison_file = str(real_comparison_file) if real_comparison_file.exists() else None

# Output directory
output_dir = script_dir.parent / "outputs" / "06_all_motif_divergence"
...
if real_comparison_file and os.path.exists(real_comparison_file):
...
for transition in all_transitions:
    plot_true_and_false(transition, true_data.get(transition, {}), false_data.get(transition, {}))
