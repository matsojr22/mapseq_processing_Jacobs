...
parser.add_argument(
    ...
    type=float,
    default=-1,
    help="Maximum number of barcodes to keep. If -1, keep all barcodes. Otherwise, randomly sample to this number.",
)
parser.add_argument(
...
def clean_and_filter(
    ...
    - Optionally randomly sample to max_barcodes limit
    """
    ...
    # 🚨 Step 8: Randomly sample all barcodes to the max_barcodes value (ADAMSEDIT)
    if max_barcodes > 0:
        current_num_barcodes = matrix.shape[0]
        if current_num_barcodes < max_barcodes:
            raise ValueError(
                f"❌ ERROR Step 8: Insufficient barcodes remaining after filtering. "
                f"Current barcodes: {current_num_barcodes}, Required: {int(max_barcodes)}. "
                f"Consider lowering max_barcodes or adjusting filtering parameters."
            )
        elif current_num_barcodes > max_barcodes:
            # Set random seed for reproducibility
            np.random.seed(42)
            # Randomly sample indices
            sampled_indices = np.random.choice(
                current_num_barcodes, size=int(max_barcodes), replace=False
            )
            matrix = matrix[sampled_indices]
            print(
                f"✅ CHECK THIS Step 8: Randomly sampled {int(max_barcodes)} barcodes from {current_num_barcodes}. Shape: {matrix.shape}"
            )
        else:
            print(
                f"✅ CHECK THIS Step 8: No sampling needed. Current barcodes ({current_num_barcodes}) equals max_barcodes ({int(max_barcodes)}). Shape: {matrix.shape}"
            )
    else:
        print("✅ CHECK THIS Step 8: Skipping barcode sampling (max_barcodes <= 0).")
